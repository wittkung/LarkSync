...
import { SyncProgressReporter } from './core/syncProgressReporter';
import { FullSyncStrategy, IncrementalSyncStrategy } from './core/syncStrategies';
...
export class SyncEngine {
    ...
    private _onProgress = new vscode.EventEmitter<{
    ...
    public async startSync(silent: boolean = false, forceFullTree: boolean = false) {
        ...
        const syncDirName =
            configuredDir !== undefined ? configuredDir : CONSTANTS.DEFAULT_SYNC_DIR;
        ...
        try {
            ...
            await vscode.window.withProgress(
                ...
                async rawProgress => {
                    ...
                    const strategy = forceFullTree
                        ? new FullSyncStrategy(this.feishuClient)
                        : new IncrementalSyncStrategy(this.feishuClient);
     ...
     * 同步所有文档节点
     */
    private async syncNodes(nodes: WikiNode[], reporter: SyncProgressReporter) {
    ...
    private async fetchDocBlocks(documentId: string, title: string): Promise<DocxBlock[]> {
        ...
        while (hasMore) {
            ...
            try {
                const response = await this.feishuClient.request<DocxBlockListResponse>({
