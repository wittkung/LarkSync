...
export interface RenderContext {
    ...
    /** Block ID → Block 对象的索引映射 */
    blockMap: Map<string, DocxBlock>;
    /** 收集到的媒体 Token 列表（由 Image/File Handler 填充） */
    ...
    /** 递归渲染单个块的入口函数 */
    renderBlock: (block: DocxBlock, ctx: RenderContext) => string;
}
...
export type BlockHandler = (block: DocxBlock, ctx: RenderContext) => string;

/**
 * 创建 Block Type → Handler 的映射注册表
 */
export function createHandlerMap(): Map<number, BlockHandler> {
    const map = new Map<number, BlockHandler>();

    // 页面根节点 — 渲染所有子块
    map.set(BlockType.PAGE, handlePage);

    // 文本类块
    map.set(BlockType.TEXT, handleText);
    map.set(BlockType.HEADING1, createHeadingHandler(1));
    map.set(BlockType.HEADING2, createHeadingHandler(2));
    map.set(BlockType.HEADING3, createHeadingHandler(3));
    map.set(BlockType.HEADING4, createHeadingHandler(4));
    map.set(BlockType.HEADING5, createHeadingHandler(5));
    map.set(BlockType.HEADING6, createHeadingHandler(6));
    map.set(BlockType.HEADING7, createHeadingHandler(7));
    map.set(BlockType.HEADING8, createHeadingHandler(8));
    map.set(BlockType.HEADING9, createHeadingHandler(9));

    // 列表类块
    map.set(BlockType.BULLET, handleBullet);
    map.set(BlockType.ORDERED, handleOrdered);
    map.set(BlockType.TODO, handleTodo);

    // 代码块
    map.set(BlockType.CODE, handleCode);

    // 引用与容器
    map.set(BlockType.QUOTE, handleQuote);
    map.set(BlockType.QUOTE_CONTAINER, handleQuoteContainer);
    map.set(BlockType.CALLOUT, handleCallout);

    // 分割线
    map.set(BlockType.DIVIDER, handleDivider);

    // 媒体
    map.set(BlockType.IMAGE, handleImage);
    map.set(BlockType.FILE, handleFile);

    // 表格
    map.set(BlockType.TABLE, handleTable);
    map.set(BlockType.TABLE_CELL, handleTableCell);

    // 分栏
    map.set(BlockType.GRID, handleGrid);
    map.set(BlockType.GRID_COLUMN, handleGridColumn);

    // 嵌入
    map.set(BlockType.IFRAME, handleIframe);

    // 视图块（仅作为容器透传子块）
    map.set(BlockType.VIEW, handlePassthrough);
 ...
 */
function applyInlineStyles(text: string, style: TextElementStyle): string {
    // 空白文本不应用样式（避免产生 `****` 等空标记）
...
// --- 页面根节点 ---
function handlePage(block: DocxBlock, ctx: RenderContext): string {
...
function createHeadingHandler(level: number): BlockHandler {
    return (block: DocxBlock, ctx: RenderContext): string => {
        ...
        const dataKey = `heading${level}` as keyof DocxBlock;
        const data = block[dataKey] as any;
        if (!data) return '';
...
// --- 无序列表 ---
function handleBullet(block: DocxBlock, ctx: RenderContext): string {
...
// --- 有序列表 ---
function handleOrdered(block: DocxBlock, ctx: RenderContext): string {
...
// --- 待办事项 ---
function handleTodo(block: DocxBlock, ctx: RenderContext): string {
...
// --- 引用块（单行引用） ---
function handleQuote(block: DocxBlock, ctx: RenderContext): string {
...
// --- 引用容器 ---
function handleQuoteContainer(block: DocxBlock, ctx: RenderContext): string {
    if (!block.children || block.children.length === 0) return '';
...
// --- 高亮块 (Callout) ---
function handleCallout(block: DocxBlock, ctx: RenderContext): string {
...
// --- 分割线 ---
function handleDivider(_block: DocxBlock, _ctx: RenderContext): string {
    return '---\n\n';
}

// --- 图片块 ---
function handleImage(block: DocxBlock, ctx: RenderContext): string {
...
// --- 附件文件块 ---
function handleFile(block: DocxBlock, ctx: RenderContext): string {
...
// --- 表格单元格 ---
function handleTableCell(block: DocxBlock, ctx: RenderContext): string {
    if (!block.children || block.children.length === 0) return '';
...
// --- 分栏块 ---
function handleGrid(block: DocxBlock, ctx: RenderContext): string {
    if (!block.children || block.children.length === 0) return '';
...
// --- 分栏列 ---
function handleGridColumn(block: DocxBlock, ctx: RenderContext): string {
    if (!block.children || block.children.length === 0) return '';
    return ctx.renderChildren(block.children, ctx);
}
...
// --- 透传容器（仅渲染子块） ---
function handlePassthrough(block: DocxBlock, ctx: RenderContext): string {
    if (!block.children || block.children.length === 0) return '';
    return ctx.renderChildren(block.children, ctx);
}
