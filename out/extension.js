...
var CONSTANTS = {
  ...
  FALLBACK_CACHE_TTL_MS: 43200 * 1e3,
...
var Logger = class _Logger {
  constructor() {
    this.channel = vscode2.window.createOutputChannel("LarkSync");
  }
  static getInstance() {
    if (!_Logger.instance) {
...
var FileManager = class {
  ...
  async ensureRootDirectory() {
    try {
      await vscode3.workspace.fs.createDirectory(this.rootUri);
  ...
  getDocLocalUri(doc) {
    ...
    return vscode3.Uri.joinPath(this.rootUri, ...parts);
  }
  /**
   ...
   */
  async readFile(uri) {
    const data = await vscode3.workspace.fs.readFile(uri);
  ...
  async writeFile(uri, content) {
    const data = Buffer.from(content, "utf-8");
    await vscode3.workspace.fs.writeFile(uri, data);
  }
  /**
   * 删除文件
   */
  async deleteFile(uri) {
    try {
      await vscode3.workspace.fs.delete(uri, { recursive: false });
    } catch {
    }
  }
  /**
  ...
  async walkDirectory(dirUri, results) {
    try {
      const entries = await vscode3.workspace.fs.readDirectory(dirUri);
      for (const [name, type] of entries) {
        const childUri = vscode3.Uri.joinPath(dirUri, name);
        if (type === vscode3.FileType.Directory) {
          if (name === "assets" || name.startsWith(".")) continue;
          await this.walkDirectory(childUri, results);
        } else if (type === vscode3.FileType.File && name.endsWith(".md")) {
  ...
  async removeEmptyDirs(dirUri) {
    ...
    try {
      ...
      if (dirUri.toString() !== this.rootUri.toString()) {
        const remaining = await vscode3.workspace.fs.readDirectory(dirUri);
        if (remaining.length === 0) {
          logger.info(`\u5220\u9664\u7A7A\u76EE\u5F55: ${dirUri.fsPath}`);
          await vscode3.workspace.fs.delete(dirUri, { recursive: false });
          count++;
        }
      }
    } catch {
  ...
  getDirectoryUriForNode(node) {
    ...
    return vscode3.Uri.joinPath(this.rootUri, ...dirParts);
  }
...
var MediaManager = class {
  constructor(rootUri, feishuClient2) {
    /** 已下载到本地的 Token 集合（用于跨文档去重） */
    this.downloadedTokens = /* @__PURE__ */ new Set();
    this.rootUri = rootUri;
    this.feishuClient = feishuClient2;
    this.assetsUri = vscode4.Uri.joinPath(rootUri, CONSTANTS.ASSETS_DIR_NAME);
  }
  /**
  ...
  async initialize() {
    try {
      const entries = await vscode4.workspace.fs.readDirectory(this.assetsUri);
      for (const [name, type] of entries) {
        if (type === vscode4.FileType.Directory) {
          const subDir = vscode4.Uri.joinPath(this.assetsUri, name);
          const subEntries = await vscode4.workspace.fs.readDirectory(subDir);
  ...
  async processTokens(tokens, docTitle) {
    ...
    const docAssetsUri = vscode4.Uri.joinPath(this.assetsUri, sanitizedTitle);
    ...
    const limit = (0, import_p_limit.default)(CONSTANTS.MEDIA_DOWNLOAD_CONCURRENCY);
    const tasks = toDownload.map(
      (entry) => limit(async () => {
        try {
          ...
          const fileUri = vscode4.Uri.joinPath(docAssetsUri, fileName);
          await vscode4.workspace.fs.writeFile(fileUri, buffer);
...
// src/core/syncProgressReporter.ts
var SyncProgressReporter = class {
  ...
  report(message, increment, type = "progress") {
    if (this.rawProgress) {
...
// src/core/treeDiffEngine.ts
var TreeDiffEngine = class {
  constructor(feishuClient2, spaceId) {
    this.feishuClient = feishuClient2;
  ...
  async diffAndMerge(cachedNodes) {
    ...
    logger.info(
      `\u589E\u91CF Diff \u5B8C\u6210: +${added.length} / -${deleted.length} / ~${contentChanged.length} (${this.apiCallCount} \u6B21 API \u8C03\u7528, \u5408\u5E76\u540E ${mergedNodes.length} \u4E2A\u8282\u70B9)`
    );
    return {
      mergedNodes,
      addedNodes: added,
  ...
  async diffLevel(parentNodeToken, cachedByParent, cachedByToken, added, deleted, contentChanged) {
    ...
    for (const freshNode of freshNodes) {
      ...
      if (!cachedNode) {
        ...
        if (freshNode.obj_edit_time && cachedNode.obj_edit_time && freshNode.obj_edit_time !== cachedNode.obj_edit_time) {
          logger.info(
            `[\u589E\u91CF] \u6587\u6863\u6709\u66F4\u65B0: [${freshNode.title}] (${cachedNode.obj_edit_time} \u2192 ${freshNode.obj_edit_time})`
          );
   ...
   * 拉取某个父节点下的所有子节点（处理分页）
   */
  async fetchAllNodesForParent(parentNodeToken) {
    const allNodes = [];
    let pageToken = "";
    let hasMore = true;
    let pageCount = 0;
    while (hasMore) {
      ...
      if (pageCount > CONSTANTS.MAX_PAGES_PER_LEVEL) {
        logger.warn(
          `[\u589E\u91CF] \u5206\u9875\u6570\u8D85\u8FC7\u5B89\u5168\u9600 ${CONSTANTS.MAX_PAGES_PER_LEVEL}\uFF0C\u505C\u6B62\u62C9\u53D6 (parent: ${parentNodeToken || "root"})`
        );
        break;
      }
   ...
   */
  collectDescendantTokens(nodeToken, cachedByParent, result) {
   ...
   */
  buildMergedTree(originalCachedNodes, updatedByToken, addedNodes, deletedTokens) {
    const deletedSet = new Set(deletedTokens);
    const surviving = originalCachedNodes.filter((n) => !deletedSet.has(n.node_token)).map((n) => updatedByToken.get(n.node_token) || n);
...
var FullSyncStrategy = class {
  constructor(feishuClient2) {
    this.feishuClient = feishuClient2;
  }
  async fetchNodes(spaceId, cachedNodes, reporter) {
...
var IncrementalSyncStrategy = class {
  constructor(feishuClient2) {
    this.feishuClient = feishuClient2;
  }
  async fetchNodes(spaceId, cachedNodes, reporter) {
...
// src/utils/errors.ts
var LarkSyncError = class extends Error {
  constructor(message) {
    super(message);
    this.name = this.constructor.name;
  }
};
var FeishuAuthError = class extends LarkSyncError {
  constructor(message) {
    super(`Feishu Auth Error: ${message}`);
  }
};
var FeishuApiError = class extends LarkSyncError {
...
var RateLimitError = class extends FeishuApiError {
  constructor(message = "Too Many Requests", code = 429) {
    super(message, code);
  }
};

// src/utils/fileWriteStrategy.ts
var BOUNDARY_START = "<!-- LARKSYNC:START -->";
var BOUNDARY_END = "<!-- LARKSYNC:END -->";
function applyProtectedWrite(existingContent, newContent) {
  if (!existingContent) {
...
var SyncEngine = class {
  constructor(feishuClient2) {
    this._onProgress = new vscode5.EventEmitter();
  ...
  async startSync(silent = false, forceFullTree = false) {
    logger.info("\u540C\u6B65\u89E6\u53D1\u3002");
    const config = vscode5.workspace.getConfiguration("larksync");
    const spaceId = config.get("spaceId");
    if (!spaceId) {
      logger.error("\u540C\u6B65\u4E2D\u6B62: \u672A\u914D\u7F6E Space ID\u3002");
      if (!silent) {
        vscode5.window.showErrorMessage("LarkSync: \u8BF7\u5728\u8BBE\u7F6E\u4E2D\u914D\u7F6E Space ID\u3002");
      }
      return;
    }
    ...
    if (!workspaceFolders || workspaceFolders.length === 0) {
      ...
      if (!silent) {
        vscode5.window.showErrorMessage("LarkSync: \u8BF7\u6253\u5F00\u4E00\u4E2A\u5DE5\u4F5C\u533A\u76EE\u5F55\u3002");
      }
      return;
    }
    ...
    const rootUri = syncDirName ? vscode5.Uri.joinPath(workspaceFolders[0].uri, syncDirName) : workspaceFolders[0].uri;
    ...
    this.mediaManager = new MediaManager(rootUri, this.feishuClient);
    await this.fileManager.ensureRootDirectory();
    await this.stateManager.loadState();
    await this.mediaManager.initialize();
    const treeCacheUri = vscode5.Uri.joinPath(rootUri, CONSTANTS.TREE_CACHE_FILE_NAME);
    try {
      logger.info("\u521D\u59CB\u5316\u8FDB\u5EA6\u7A97\u53E3...");
      await vscode5.window.withProgress(
        {
          location: silent ? vscode5.ProgressLocation.Window : vscode5.ProgressLocation.Notification,
          title: "LarkSync",
          cancellable: false
        },
        async (rawProgress) => {
          const reporter = new SyncProgressReporter(this._onProgress);
          reporter.setRawProgress(rawProgress);
          reporter.report("Initializing sync...", 0, "start");
          let nodes = [];
          const deleteOrphans = config.get("deleteOrphanFiles") ?? true;
          let treeIsComplete = false;
          let cachedNodes = [];
          if (!forceFullTree) {
            try {
              const cacheData = await vscode5.workspace.fs.readFile(treeCacheUri);
          ...
          const strategy = forceFullTree ? new FullSyncStrategy(this.feishuClient) : new IncrementalSyncStrategy(this.feishuClient);
          const result = await strategy.fetchNodes(spaceId, cachedNodes, reporter);
          nodes = result.nodes;
          treeIsComplete = result.isComplete;
          const treeCacheContent = Buffer.from(JSON.stringify(nodes, null, 2), "utf-8");
          await vscode5.workspace.fs.writeFile(treeCacheUri, treeCacheContent);
          logger.info(`\u5DF2\u7F13\u5B58 ${nodes.length} \u4E2A\u8282\u70B9\u3002`);
          reporter.report(`\u51C6\u5907\u540C\u6B65 ${nodes.length} \u4E2A\u8282\u70B9...`);
          this.fileManager.setNodes(nodes);
          await this.syncNodes(nodes, reporter);
          if (deleteOrphans && treeIsComplete) {
            reporter.report("\u6B63\u5728\u6E05\u7406\u4E91\u7AEF\u5DF2\u5220\u9664\u7684\u672C\u5730\u6587\u4EF6...");
      ...
      vscode5.commands.executeCommand("larksync.refreshSidebar");
      if (!silent) {
        vscode5.window.showInformationMessage("LarkSync: \u540C\u6B65\u5B8C\u6210\uFF01");
      }
    } catch (error) {
      this._onProgress.fire({ type: "error", message: error.message });
      logger.error(`\u540C\u6B65\u9519\u8BEF: ${error.message}`, error);
      if (!silent) {
        vscode5.window.showErrorMessage(`LarkSync \u9519\u8BEF: ${error.message}`);
      }
    }
  }
  /**
   * 同步所有文档节点
   */
  async syncNodes(nodes, reporter) {
    const limit = (0, import_p_limit2.default)(CONSTANTS.CONCURRENCY_LIMIT);
    ...
    reporter.report(`\u6B63\u5728\u6821\u9A8C ${docs.length} \u7BC7\u6587\u6863\u7684\u4E91\u7AEF\u72B6\u6001...`);
    logger.info("\u6279\u91CF\u83B7\u53D6\u6587\u6863\u5143\u6570\u636E...");
    const metaTokens = docs.map((d) => ({
      token: d.obj_token,
      type: d.obj_type,
      title: d.title
    }));
    const cloudMetas = await this.feishuClient.fetchDocsMetadata(metaTokens);
    const tasks = docs.map(
      (doc) => limit(async () => {
        reporter.report(`\u540C\u6B65: ${doc.title}`, incrementValue);
        ...
        if (fileExists && this.stateManager.getDocState(doc.obj_token)) {
          const cloudUnchanged = this.stateManager.isDocUnchanged(
            doc.obj_token,
            cloudMeta?.latest_modify_time
          );
          let isModifiedLocally = false;
          try {
            const stats = await vscode5.workspace.fs.stat(targetUri);
            const state = this.stateManager.getDocState(doc.obj_token);
            if (state && stats.mtime - state.lastSyncTime > 2e3) {
          ...
          if (cloudUnchanged) {
            ...
            logger.warn(
              `\u51B2\u7A81\u8DF3\u8FC7: [${doc.title}] \u672C\u5730\u5DF2\u88AB\u4FEE\u6539\uFF0C\u6682\u505C\u62C9\u53D6\u4EE5\u4FDD\u62A4\u60A8\u7684\u6570\u636E\u3002`
            );
            vscode5.window.showWarningMessage(
              `LarkSync: [${doc.title}] \u5B58\u5728\u51B2\u7A81\uFF01\u672C\u5730\u4FEE\u6539\u5DF2\u4FDD\u7559\uFF0C\u6682\u65F6\u505C\u6B62\u4E91\u7AEF\u8986\u76D6\u3002`
            );
            return;
          }
        }
    ...
    while (hasMore) {
      ...
      try {
        ...
        if (err instanceof FeishuApiError) {
          throw new Error(
            `API Error [${err.code}]: ${err.message} \u2014 \u8BF7\u786E\u8BA4\u5E94\u7528\u62E5\u6709 docx:document:readonly \u6743\u9650\u3002`
          );
        }
        throw err;
      }
...
var SyncScheduler = class {
  ...
  async requestSync(silent = false, forceFullTree = false) {
    if (this.syncPromise) {
      logger.info("\u540C\u6B65\u5DF2\u5728\u8FDB\u884C\u4E2D\uFF0C\u6392\u961F\u5904\u7406...");
      if (!silent) {
        vscode6.window.showInformationMessage("LarkSync: \u540C\u6B65\u6B63\u5728\u8FDB\u884C\u4E2D\uFF0C\u5DF2\u4E3A\u60A8\u6392\u961F\u3002");
      }
      if (this.nextSyncScheduled) {
        this.nextSyncScheduled.forceFullTree = this.nextSyncScheduled.forceFullTree || forceFullTree;
  ...
  async runSync(silent, forceFullTree) {
  ...
  async getChildren(element) {
    ...
    try {
      ...
      for (const [name, fileType] of entries) {
        ...
        const isDir = fileType === vscode7.FileType.Directory;
        if (isDir) {
          nodes.push(
            new SyncNode(name, childUri, vscode7.TreeItemCollapsibleState.Collapsed)
          );
        } else {
...
var SyncNode = class extends vscode7.TreeItem {
  constructor(label, uri, collapsibleState, command, description, customTooltip) {
    ...
    } else if (collapsibleState !== vscode7.TreeItemCollapsibleState.None) {
      this.iconPath = vscode7.ThemeIcon.Folder;
    }
  }
};

// src/ui/syncFileDecorationProvider.ts
var vscode8 = __toESM(require("vscode"));
var SyncFileDecorationProvider = class {
  constructor() {
  ...
  provideFileDecoration(uri) {
    ...
    switch (status) {
      case "local_modified" /* LOCAL_MODIFIED */:
        return {
          badge: "M",
          color: new vscode8.ThemeColor("gitDecoration.modifiedResourceForeground"),
          tooltip: "\u672C\u5730\u5DF2\u88AB\u4FEE\u6539"
        };
      case "cloud_updated" /* CLOUD_UPDATED */:
        return {
          badge: "U",
          color: new vscode8.ThemeColor("gitDecoration.untrackedResourceForeground"),
          // Often blue/green
          tooltip: "\u4E91\u7AEF\u6709\u65B0\u7248\u672C"
        };
      case "conflict" /* CONFLICT */:
        return {
          badge: "C",
          color: new vscode8.ThemeColor("gitDecoration.conflictingResourceForeground"),
          tooltip: "\u53CC\u7AEF\u5747\u88AB\u4FEE\u6539\uFF0C\u5B58\u5728\u51B2\u7A81\uFF01"
        };
      ...
      case "added" /* ADDED */:
        return {
          badge: "A",
          color: new vscode8.ThemeColor("gitDecoration.addedResourceForeground"),
          tooltip: "\u8FDC\u7AEF\u65B0\u589E\u6587\u4EF6"
        };
      case "deleted" /* DELETED */:
        return {
          badge: "D",
          color: new vscode8.ThemeColor("gitDecoration.deletedResourceForeground"),
          tooltip: "\u8FDC\u7AEF\u5DF2\u5220\u9664"
        };
      default:
        return void 0;
...
var KEY_TOKEN_EXPIRY = "larksync.tokenExpiryTime";
...
var OAuthManager = class {
  ...
  async refreshUserToken() {
    ...
    try {
      ...
      if (data?.access_token) {
        ...
        await this.tokenStore.setTokenExpiry(Date.now() + expiresIn * 1e3);
  ...
  async getAppAccessToken(appId, appSecret) {
    const response = await axios_default.post(CONSTANTS.FEISHU_AUTH_URL, {
      app_id: appId,
  ...
  generateCodeVerifier() {
    return crypto2.randomBytes(32).toString("base64url").slice(0, 128);
  }
  /**
...
var RateLimiter = class {
  ...
  async acquire() {
    ...
    const waitMs = 1 / this.config.maxQPS * 1e3;
  ...
  async executeWithRetry(fn, retryOnError) {
    ...
    for (let attempt = 0; attempt <= this.config.maxRetries; attempt++) {
      ...
      try {
        ...
        logger.warn(
          `\u8BF7\u6C42\u5931\u8D25 (attempt ${attempt + 1}/${this.config.maxRetries + 1}), ${delayMs}ms \u540E\u91CD\u8BD5: ${err.message}`
        );
  ...
  refill() {
    const now = Date.now();
    const elapsedSec = (now - this.lastRefillTime) / 1e3;
  ...
  async getTenantAccessToken() {
    ...
    this.pendingTokenRefresh = (async () => {
      try {
        ...
        const response = await axios_default.post(CONSTANTS.FEISHU_AUTH_URL, { app_id: appId, app_secret: appSecret });
...
var DashboardPanel = class _DashboardPanel {
  constructor(panel, extensionUri, syncEngine2, feishuClient2) {
    this._disposables = [];
    this._panel = panel;
  ...
  static createOrShow(extensionUri, syncEngine2, feishuClient2) {
    const column = vscode11.window.activeTextEditor ? vscode11.window.activeTextEditor.viewColumn : void 0;
    ...
    const panel = vscode11.window.createWebviewPanel(
      "larksyncDashboard",
      "LarkSync Dashboard",
      column || vscode11.ViewColumn.One,
      {
        enableScripts: true,
        localResourceRoots: [vscode11.Uri.joinPath(extensionUri, "webview-ui", "build")],
        retainContextWhenHidden: true
        // Keep React state alive
      }
    );
    _DashboardPanel.currentPanel = new _DashboardPanel(
      panel,
      extensionUri,
      syncEngine2,
      feishuClient2
  ...
  async _handleMessage(message) {
    ...
    switch (command) {
      ...
      case "openSettings":
        ...
        return;
      case "openFolder":
        vscode11.commands.executeCommand("larksync.openSyncFolder");
        return;
      case "importCacheFiles":
        vscode11.commands.executeCommand("larksync.importCacheFiles");
        return;
      case "startSync":
        vscode11.commands.executeCommand("larksync.startSync");
        return;
      case "login": {
        vscode11.commands.executeCommand("larksync.login");
      ...
      case "openExternalUrl":
        if (payload && typeof payload === "string") {
          vscode11.env.openExternal(vscode11.Uri.parse(payload));
        }
        return;
      case "saveConfig":
        if (payload && payload.key && typeof payload.value === "string") {
          await vscode11.workspace.getConfiguration().update(
            payload.key,
            payload.value,
            vscode11.workspace.workspaceFolders ? vscode11.ConfigurationTarget.Workspace : vscode11.ConfigurationTarget.Global
          );
          this.sendHealthData();
        }
        return;
  ...
  sendHealthData() {
    ...
    try {
      if (this.syncEngine && this.syncEngine.stateManager) {
        ...
        if (state) {
          const times = Object.values(state).map(
            (e) => e.lastSyncTime || 0
          );
          if (times.length > 0) {
  ...
  _getHtmlForWebview(webview) {
    const scriptUri = webview.asWebviewUri(
      vscode11.Uri.joinPath(this._extensionUri, "webview-ui", "build", "assets", "index.js")
    );
    const styleUri = webview.asWebviewUri(
      vscode11.Uri.joinPath(this._extensionUri, "webview-ui", "build", "assets", "index.css")
    );
...
function activate(context) {
  try {
    ...
    context.subscriptions.push(
      vscode12.window.registerTreeDataProvider("larksyncSidebar", treeProvider)
    );
    decorationProvider = new SyncFileDecorationProvider();
    context.subscriptions.push(
      vscode12.window.registerFileDecorationProvider(decorationProvider)
    );
    statusBarItem = vscode12.window.createStatusBarItem(vscode12.StatusBarAlignment.Right, 100);
    ...
    context.subscriptions.push(
      vscode12.commands.registerCommand("larksync.startSync", async () => {
    ...
    context.subscriptions.push(
      vscode12.commands.registerCommand("larksync.forceSyncTree", async () => {
    ...
    context.subscriptions.push(
      vscode12.commands.registerCommand("larksync.openSyncFolder", () => {
        const workspaceFolders = vscode12.workspace.workspaceFolders;
        if (workspaceFolders && workspaceFolders.length > 0) {
          const config = vscode12.workspace.getConfiguration("larksync");
          const syncDirName = config.get("syncDir", CONSTANTS.DEFAULT_SYNC_DIR);
          const targetUri = vscode12.Uri.joinPath(workspaceFolders[0].uri, syncDirName);
          vscode12.commands.executeCommand("revealFileInOS", targetUri);
        } else {
          vscode12.window.showErrorMessage("LarkSync: \u6CA1\u6709\u6253\u5F00\u7684\u5DE5\u4F5C\u533A\u3002");
        }
      })
    );
    context.subscriptions.push(
      vscode12.commands.registerCommand("larksync.refreshSidebar", () => {
        if (treeProvider) {
    ...
    context.subscriptions.push(
      vscode12.commands.registerCommand("larksync.openDashboard", () => {
    ...
    try {
      ...
      context.subscriptions.push(vscode12.window.registerUriHandler(oauthManager));
      context.subscriptions.push(
        vscode12.commands.registerCommand("larksync.login", async () => {
      ...
      context.subscriptions.push(
        vscode12.commands.registerCommand("larksync.logout", async () => {
  ...
  if (intervalMinutes > 0) {
    ...
    pollingTimer = setInterval(async () => {
      if (!syncScheduler.isSyncing) {
        ...
        await syncScheduler.requestSync(true, false);
        statusBarItem.text = "$(sync) LarkSync";
      }
    }, intervalMs);
  }
