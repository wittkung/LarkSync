...
var StateManager = class {
  constructor(rootUri) {
    this.state = {};
    this.stateUri = vscode3.Uri.joinPath(rootUri, CONSTANTS.STATE_FILE_NAME);
  }
  /**
   * 从工作区文件系统加载同步状态
   */
  async loadState() {
    try {
      const data = await vscode3.workspace.fs.readFile(this.stateUri);
  ...
  async saveState() {
    ...
    await vscode3.workspace.fs.writeFile(this.stateUri, data);
  }
...
var FileManager = class {
  ...
  async ensureRootDirectory() {
    try {
      await vscode4.workspace.fs.createDirectory(this.rootUri);
   ...
   */
  async readFile(uri) {
    const data = await vscode4.workspace.fs.readFile(uri);
  ...
  async writeFile(uri, content) {
    const data = Buffer.from(content, "utf-8");
    await vscode4.workspace.fs.writeFile(uri, data);
  }
  /**
   * 删除文件
   */
  async deleteFile(uri) {
    try {
      await vscode4.workspace.fs.delete(uri, { recursive: false });
    } catch {
    }
  }
  /**
  ...
  async removeEmptyDirs(dirUri) {
    ...
    try {
      ...
      if (dirUri.toString() !== this.rootUri.toString()) {
        const remaining = await vscode4.workspace.fs.readDirectory(dirUri);
        if (remaining.length === 0) {
          logger.info(`\u5220\u9664\u7A7A\u76EE\u5F55: ${dirUri.fsPath}`);
          await vscode4.workspace.fs.delete(dirUri, { recursive: false });
          count++;
        }
      }
    } catch {
...
var MediaManager = class {
  constructor(rootUri) {
    ...
    this.rootUri = rootUri;
    this.assetsUri = vscode5.Uri.joinPath(rootUri, CONSTANTS.ASSETS_DIR_NAME);
  }
  /**
  ...
  async initialize() {
    try {
      const entries = await vscode5.workspace.fs.readDirectory(this.assetsUri);
      for (const [name, type] of entries) {
        if (type === vscode5.FileType.Directory) {
          const subDir = vscode5.Uri.joinPath(this.assetsUri, name);
          const subEntries = await vscode5.workspace.fs.readDirectory(subDir);
  ...
  async processTokens(tokens, docTitle) {
    ...
    const docAssetsUri = vscode5.Uri.joinPath(this.assetsUri, sanitizedTitle);
    ...
    const tasks = toDownload.map((entry) => limit(async () => {
      try {
        ...
        const fileUri = vscode5.Uri.joinPath(docAssetsUri, fileName);
        await vscode5.workspace.fs.writeFile(fileUri, buffer);
  ...
  async ensureDirectory(uri) {
    try {
      await vscode5.workspace.fs.createDirectory(uri);
    } catch {
    }
  }
  /**
...
var SyncEngine = class {
  constructor() {
    ...
    this._onProgress = new vscode6.EventEmitter();
  ...
  async startSync(silent = false, forceFullTree = false) {
    ...
    if (this.isSyncing) {
      logger.warn("\u540C\u6B65\u6B63\u5728\u8FDB\u884C\u4E2D\u3002");
      if (!silent) {
        vscode6.window.showWarningMessage("LarkSync: \u540C\u6B65\u6B63\u5728\u8FDB\u884C\u4E2D\u3002");
      }
      return;
    }
    const config = vscode6.workspace.getConfiguration("larksync");
    const spaceId = config.get("spaceId");
    if (!spaceId) {
      logger.error("\u540C\u6B65\u4E2D\u6B62: \u672A\u914D\u7F6E Space ID\u3002");
      if (!silent) {
        vscode6.window.showErrorMessage("LarkSync: \u8BF7\u5728\u8BBE\u7F6E\u4E2D\u914D\u7F6E Space ID\u3002");
      }
      return;
    }
    ...
    if (!workspaceFolders || workspaceFolders.length === 0) {
      ...
      if (!silent) {
        vscode6.window.showErrorMessage("LarkSync: \u8BF7\u6253\u5F00\u4E00\u4E2A\u5DE5\u4F5C\u533A\u76EE\u5F55\u3002");
      }
      return;
    }
    this.isSyncing = true;
    ...
    const rootUri = syncDirName ? vscode6.Uri.joinPath(workspaceFolders[0].uri, syncDirName) : workspaceFolders[0].uri;
    ...
    const treeCacheUri = vscode6.Uri.joinPath(rootUri, CONSTANTS.TREE_CACHE_FILE_NAME);
    try {
      logger.info("\u521D\u59CB\u5316\u8FDB\u5EA6\u7A97\u53E3...");
      await vscode6.window.withProgress({
        location: silent ? vscode6.ProgressLocation.Window : vscode6.ProgressLocation.Notification,
        title: "LarkSync",
        cancellable: false
      }, async (rawProgress) => {
        ...
        if (forceFullTree) {
          ...
          let cachedNodes = [];
          try {
            const cacheData = await vscode6.workspace.fs.readFile(treeCacheUri);
        ...
        await vscode6.workspace.fs.writeFile(treeCacheUri, treeCacheContent);
      ...
      vscode6.commands.executeCommand("larksync.refreshSidebar");
      if (!silent) {
        vscode6.window.showInformationMessage("LarkSync: \u540C\u6B65\u5B8C\u6210\uFF01");
      }
    } catch (error) {
      this._onProgress.fire({ type: "error", message: error.message });
      logger.error(`\u540C\u6B65\u9519\u8BEF: ${error.message}`, error);
      if (!silent) {
        vscode6.window.showErrorMessage(`LarkSync \u9519\u8BEF: ${error.message}`);
      }
    } finally {
      this.isSyncing = false;
    }
  ...
  async syncNodes(nodes, progress) {
    ...
    const tasks = docs.map((doc) => limit(async () => {
      ...
      if (fileExists && this.stateManager.getDocState(doc.obj_token)) {
        ...
        try {
          const stats = await vscode6.workspace.fs.stat(targetUri);
        ...
        if (cloudUnchanged) {
          ...
          vscode6.window.showWarningMessage(`LarkSync: [${doc.title}] \u5B58\u5728\u51B2\u7A81\uFF01\u672C\u5730\u4FEE\u6539\u5DF2\u4FDD\u7559\uFF0C\u6682\u65F6\u505C\u6B62\u4E91\u7AEF\u8986\u76D6\u3002`);
          return;
        }
      }
  ...
  if (intervalMinutes > 0) {
    ...
    pollingTimer = setInterval(async () => {
      if (!syncEngine.syncActive) {
        ...
        await syncEngine.startSync(true, false);
        statusBarItem.text = "$(sync) LarkSync";
      }
    }, intervalMs);
  }
